...
class GraphFlowModel(nn.Module):
    ...
    def __init__(self, max_size, node_dim, bond_dim, edge_unroll, args):
        ...
        if self.node_masks is None:         #
            self.node_masks, self.adj_masks, self.link_prediction_index, self.flow_core_edge_masks = self.initialize_masks(max_node_unroll=self.max_size, max_edge_unroll=self.edge_unroll)

        self.latent_step = self.node_masks.size(0)  # 416 (max_size) + (max_edge_unroll - 1) / 2 * max_edge_unroll + (max_size - max_edge_unroll) * max_edge_unroll
        self.latent_node_length = self.max_size * self.node_dim  # 342
        self.latent_edge_length = (self.latent_step - self.max_size) * self.bond_dim  # 1512
    ...
    def forward(self, inp_node_features, inp_adj_features, seg_mol):
        """
        Args:
        ...
        Returns:
            ...
            logdet:  ([B], [B])        
        """
        inp_seg_mol_features = inp_adj_features * seg_mol.unsqueeze(axis=1)
        inp_node_features_cont = inp_node_features.clone() #(B, N, 9) #! this is buggy. shallow copy
        inp_adj_features_cont = inp_adj_features[:, :, self.flow_core_edge_masks].clone()   #(B, 4, edge_num)   torch.Size([32, 4, 378])
        inp_adj_features_cont = inp_adj_features_cont.permute(0, 2, 1).contiguous()         #(B, edge_num, 4)   torch.Size([32, 378, 4])


        if self.args.deq_type == 'random':

            inp_node_features_cont += 0.45 * torch.rand(inp_node_features_cont.size()).cuda()                    #(B, N, 9)
            inp_adj_features_cont += 0.45 * torch.rand(inp_adj_features_cont.size()).cuda()                      #(B, edge_num, 4)
        ...
        local_vector, local_vector_unique = self.flow_core.get_graph_embs(inp_node_features, inp_seg_mol_features)
        # z, logdet, out_vector, out_atom_vector = self.flow_core(inp_node_features, inp_adj_features, inp_node_features_cont, inp_adj_features_cont)
        z, logdet, global_vector = self.flow_core(inp_node_features, inp_adj_features, inp_node_features_cont, inp_adj_features_cont)


        if self.args.deq_type == 'random':
            # return z, logdet, self.prior_ln_var, out_vector, out_atom_vector
            return z, logdet, self.prior_ln_var, global_vector, local_vector, local_vector_unique
    ...
    def generate_one_molecule_with_latent_provided(self, latent_manipulated, latent_org, max_step, max_atoms=48, temperature=0.75):
        """
        ...
        with torch.no_grad():
            ...
            for i in range(max_atoms):
                ...
                if i < self.max_size and i < max_step: # node less than 38, use pre latent
                    latent_node = org_latent_node[self.node_dim * i: self.node_dim * (i+1)].clone().view(1, -1) #(1, 9)
                elif i < self.max_size and i >= max_step:
                    latent_node = m_latent_node[self.node_dim * i: self.node_dim * (i+1)].clone().view(1, -1) #(1, 9)
                        ...
                        if len(invalid_bond_type_set) < 3 and resample_edge <= 50: # haven't sampled all possible bond type or is not stuck in the loop
                            if first:
                                ...
                                if i < self.max_size and i< max_step:
                                    latent_edge = org_latent_edge[self.bond_dim * total_edge_index: self.bond_dim * (total_edge_index + 1)].clone().view(1, -1) #(1, 4)
                                elif i < self.max_size and i>= max_step:
                                    latent_edge = m_latent_edge[self.bond_dim * total_edge_index: self.bond_dim * (total_edge_index + 1)].clone().view(1, -1) #(1, 4)
    ...
    def generate(self, temperature=0.75, mute=False, max_atoms=48, cnt=None):
        ...
        with torch.no_grad():
            ...
            # [6, 7, 8, 9, 15, 16, 17, 35, 53, 0]
            # zinc
            num2atom = {0: 6, 1: 7, 2: 8, 3: 9, 4: 15, 5: 16, 6: 17, 7: 35, 8: 53, 9: 0}
            # qm9
            #num2atom = {0: 6, 1: 7, 2: 8, 3: 9, 4: 0}
            # moses 
            #num2atom = {0: 6, 1: 7, 2: 8, 3: 9, 4:16, 5:17, 6:35, 7:0}
            ...
            cur_node_features = torch.zeros([1, max_atoms, self.node_dim]).cuda()               # 1 * 48 * 9
            cur_adj_features = torch.zeros([1, self.bond_dim, max_atoms, max_atoms]).cuda()     # 1 * 4 * 48 * 48
    ...
    def initialize_masks(self, max_node_unroll=38, max_edge_unroll=12):
        ...
        """
        num_masks = int(max_node_unroll + (max_edge_unroll - 1) * max_edge_unroll / 2 + (max_node_unroll - max_edge_unroll) * (max_edge_unroll)) #416
        num_mask_edge = int(num_masks - max_node_unroll)  # 378

        node_masks1 = torch.zeros([max_node_unroll, max_node_unroll]).byte()    # 38 * 38
        adj_masks1 = torch.zeros([max_node_unroll, max_node_unroll, max_node_unroll]).byte()   # 38 * 38 * 38
        node_masks2 = torch.zeros([num_mask_edge, max_node_unroll]).byte()  # 378 * 38
        adj_masks2 = torch.zeros([num_mask_edge, max_node_unroll, max_node_unroll]).byte()   # 378 * 38 * 38
        #is_node_update_masks = torch.zeros([num_masks]).byte()

        link_prediction_index = torch.zeros([num_mask_edge, 2]).long()   # 378 * 2


        flow_core_edge_masks = torch.zeros([max_node_unroll, max_node_unroll]).byte()  #38 * 38

        #masks_edge = dict()
        cnt = 0
        cnt_node = 0
        cnt_edge = 0
        for i in range(max_node_unroll):        # 38
            node_masks1[cnt_node][:i] = 1       # 38 * 38
            adj_masks1[cnt_node][:i, :i] = 1    # 38 * 38 * 38
            #is_node_update_masks[cnt] = 1
            cnt += 1
            cnt_node += 1

            edge_total = 0
            if i < max_edge_unroll:             # 12
                start = 0
                edge_total = i
            else:
    ...
    def log_prob(self, z, logdet, deq_logp=None, deq_logdet=None):
        ...
        logdet[0] = logdet[0] - self.latent_node_length # 342 calculate probability of a region from probability density, minus constant has no effect on optimization
        logdet[1] = logdet[1] - self.latent_edge_length # 1512 calculate probability of a region from probability density, minus constant has no effect on optimization

        ll_node = -1/2 * (torch.log(2 * self.constant_pi) + self.prior_ln_var + torch.exp(-self.prior_ln_var) * (z[0]**2)) # 32 * 342
        ll_node = ll_node.sum(-1) # (B)

        ll_edge = -1/2 * (torch.log(2 * self.constant_pi) + self.prior_ln_var + torch.exp(-self.prior_ln_var) * (z[1]**2)) # 32 * 1512
        ll_edge = ll_edge.sum(-1) # (B)
    ...
    def mutual_info(self, global_vector, local_vector, local_vector_unique):

        num_graphs = global_vector.shape[0]
        num_nodes = local_vector_unique.shape[0]
        #local_vector = local_vector.detach()

        pos_mask = torch.zeros((num_nodes, num_graphs)).cuda()
        neg_mask = torch.ones((num_nodes, num_graphs)).cuda()
        begin_index = 0
        for tem_graph, tem_node in enumerate(local_vector):
            subgraphs_num = torch.unique(tem_node, dim=0).shape[0]
            pos_mask[begin_index: begin_index + subgraphs_num][tem_graph] = 1.
            neg_mask[begin_index: begin_index + subgraphs_num][tem_graph] = 0.

        res = torch.mm(local_vector_unique, global_vector.t())
        E_pos = (F.softplus(-res * pos_mask) + res * pos_mask - math.log(2.)).mean()
        E_pos = E_pos / num_nodes
        E_neg = (F.softplus(-res * neg_mask) + res * neg_mask - math.log(2.)).mean()
