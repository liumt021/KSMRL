...
from dataloader import PretrainQm9Dataset
from dataloader import PretrainMosesDataset
...
def read_molecules(path):
    ...
    dts_features = np.load(path + '_mol_dis_atom.npy')
    dts_adj_features = np.load(path + '_mol_dis_adj.npy')
    mol_segs = np.load(path + '_mol_seg.npy')
    ...
    all_smiles = []
    fp = np.load(path + '_all_smiles.npy')
    for smiles in fp:
        all_smiles.append(smiles)

    return node_features, adj_features, mol_sizes, data_config, all_smiles, dts_features, dts_adj_features, mol_segs
...
class Trainer(object):
    ...
    def initialize_from_checkpoint(self, gen=False):
        ...
        self._model.load_state_dict(checkpoint['model_state_dict'], strict=False)    #原始的
        if not gen:
    ...
    def generate_molecule(self, num=100, epoch=None, out_path=None, mute=False, save_good_mol=False):
        ...
        unique_smiles = list(set(all_smiles))           # 生成的 num 个分子中，唯一的有几个
        unique_rate = len(unique_smiles) / num          # 唯一的个数 / 总的  唯一率
        pure_valid_rate = sum(pure_valids) / num        # 没有重采样的占比 一次生成有效的
        novelty = 1. - (appear_in_train / num)          # 生成的 Num 个分子中， 没有在训练集中出现的概率
    ...
    def fit(self, mol_out_dir=None):        
        t_total = time()
        total_loss = []
        best_loss = self.best_loss  # 100
        start_epoch = self.start_epoch   # 0
        all_unique_rate = []
        all_valid_rate = []
        all_novelty_rate = []
    ...
    def train_epoch(self, epoch_cnt):
        ...
        for i_batch, batch_data in enumerate(self.dataloader):
            ...
            seg_mol = batch_data['seg_mol']

            if self.args.cuda:
                inp_node_features = inp_node_features.cuda()
                inp_adj_features = inp_adj_features.cuda()
                seg_mol = seg_mol.cuda()

            if self.args.deq_type == 'random':

                out_z, out_logdet, ln_var, global_vector, local_vector, local_vector_unique = self._model(inp_node_features, inp_adj_features, seg_mol)
                loss1 = self._model.log_prob(out_z, out_logdet)
                loss2 = self._model.mutual_info(global_vector, local_vector, local_vector_unique)
                loss = loss1 + loss2
            ...
            if batch_cnt % self.args.show_loss_step == 0 or (epoch_cnt == 0 and batch_cnt <= 100):
                ...
                print('epoch: %d | step: %d | time: %.5f | loss: %.5f | loss1: %.5f | loss2: %.5f ' % (epoch_cnt, batch_cnt, time() - batch_time_s, batch_losses[-1], loss1.data.cpu().numpy(), loss2.data.cpu().numpy()))
...
if __name__ == '__main__':
    ...
    node_features, adj_features, mol_sizes, data_config, all_smiles, dts_features, dts_adj_features, mol_segs = read_molecules(args.path)

    train_dataloader = DataLoader(PretrainZinkDataset(node_features, adj_features, dts_features, dts_adj_features, mol_sizes, all_smiles, mol_segs),
