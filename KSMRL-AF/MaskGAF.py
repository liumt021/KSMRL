...
class RelationGraphConvolution(nn.Module):  # RGCN 中初始化
    """
    Relation GCN layer. 
    """
    ...
    def forward(self, x, adj):
        ...
        # transform
        # print(f'x = {x.shape}')                   #[480, 9, 4] torch.Size([3168, 9, 4])
        # print(f'weight = {self.weight.shape}')    #[3, 4, 128] torch.Size([3, 4, 128])
        support = torch.einsum('bid, edh-> beih', x, self.weight)
        # print(f'support = {support.shape}')       #   [480, 3, 9, 128] torch.Size([3168, 3, 9, 128])
        # print(f'adj = {adj.shape}')               #   [480, 3, 9, 9]  torch.Size([1408, 3, 9, 9])
        output = torch.einsum('beij, bejh-> beih', adj, support)  # (batch, e, N, d)
        # print(f'output = {output.shape}')         #   [480, 3, 9, 128]
        # sys.exit()
...
class RGCN(nn.Module):  # MaskedGraphAF 中初始化
...
class ST_Net_Exp(nn.Module):
    def __init__(self, args, input_dim, output_dim, hid_dim=64, num_layers=2, bias=True, scale_weight_norm=False, sigmoid_shift=2., apply_batch_norm=False):
        ...
        self.num_layers = num_layers  # unused
        self.args = args
        self.input_dim = input_dim                  # 128 384
        self.hid_dim = hid_dim                      # 128
        self.output_dim = output_dim                # 9   4
        self.bias = bias                            # ture
        self.apply_batch_norm = apply_batch_norm    # False
        self.scale_weight_norm = scale_weight_norm  # False
        self.sigmoid_shift = sigmoid_shift          # 2.0

        self.linear1 = nn.Linear(input_dim, hid_dim, bias=bias)     # node 128  128     edge 384 128
        self.linear2 = nn.Linear(hid_dim, output_dim*2, bias=bias)  # node 128  18      edge 128 8

        if self.apply_batch_norm:# XXX
            self.bn_before = nn.BatchNorm1d(input_dim)
        if self.scale_weight_norm:# XXX
            self.rescale1 = nn.utils.weight_norm(Rescale())
            #self.rescale2 = nn.utils.weight_norm(Rescale())
    ...
    def reset_parameters(self):
        nn.init.xavier_uniform_(self.linear1.weight)        # 通过网络层时，输入和输出的方差相同，均匀分布
        nn.init.constant_(self.linear2.weight, 1e-10)       # 初始化为常数
        if self.bias:
            nn.init.constant_(self.linear1.bias, 0.)        # 初始化为常数
            nn.init.constant_(self.linear2.bias, 0.)        # 初始化为常数
...
class MaskedGraphAF(nn.Module):   #  在 Model 中调用
    def __init__(self, mask_node, mask_edge, index_select_edge, num_flow_layer, graph_size,
        ...
        '''
        [416, 38]  [416, 38, 38]  [378, 2]  12  /  38
           9  /  4
        :param index_nod_edg:
        :param num_edge_type, virtual type included
        '''
        super(MaskedGraphAF, self).__init__()
        self.repeat_num = mask_node.size(0)             #  416
        self.graph_size = graph_size                    #  38
        self.num_node_type = num_node_type              #  9
        self.num_edge_type = num_edge_type              #  4
        self.args = args
        self.is_batchNorm = self.args.is_bn             # true
        # self.out_vectors = []
        self.out_atom_vectors = []
        self.mask_node = nn.Parameter(mask_node.view(1, self.repeat_num, graph_size, 1), requires_grad=False)  # (1, 416, 38, 1)
        self.mask_edge = nn.Parameter(mask_edge.view(1, self.repeat_num, 1, graph_size, graph_size), requires_grad=False)  # (1, 416, 1, 38, 38)
        self.index_select_edge = nn.Parameter(index_select_edge, requires_grad=False)  # (378, 2)

        self.emb_size = nout                    # 128
        self.hid_size = nhid                    # 128
        self.num_flow_layer = num_flow_layer    # 12
        #                       9                   128                 128                     3
        ...
        if self.is_batchNorm:                       # true
            self.batchNorm = nn.BatchNorm1d(nout)   # 在每层激活函数前加一个归一化
    ...
    def get_graph_embs(self, x, adj):

        graph_emb_node, _ = self._get_embs(x, adj)
        out_atom_vector = graph_emb_node.view(-1, x.shape[1], 128)
        out_letent_vec = torch.einsum('beij,bjd->bid', adj, out_atom_vector)
        out_letent_vec = out_letent_vec.detach()
        out_letent_vec_ss = torch.unique(out_letent_vec.view(-1, 128), dim=0)
        return out_letent_vec, out_letent_vec_ss


    def forward(self, x, adj, x_deq, adj_deq):
        '''
        :param x:   (batch, N 38, 9)
        :param adj: (batch, 4, N 38, N 38)

        :param x_deq: (batch, N 38, 9)
        :param adj_deq:  (batch, edge_num 378, 4)
        :return:
        '''
        # inputs for RelGCNs
        batch_size = x.size(0)
        ...
        global_vector = graph_emb_node.view(-1, x.shape[1], 128).sum(axis=1)

        x_deq = x_deq.view(-1, self.num_node_type)          # (batch *N, 9)                 torch.Size([1216, 128])
        adj_deq = adj_deq.view(-1, self.num_edge_type)      # (batch*(repeat_num-N), 4)     torch.Size([12096, 384])
        ...
        for i in range(self.num_flow_layer):
            ...
            if i == 0:
                x_log_jacob = (torch.abs(node_s) + 1e-20).log()         # torch.Size([1216, 9])
            else:
                x_log_jacob += (torch.abs(node_s) + 1e-20).log()        # torch.Size([1216, 9])
            ...
            if i == 0:
                adj_log_jacob = (torch.abs(edge_s) + 1e-20).log()       # torch.Size([12096, 4])
            else:
                adj_log_jacob += (torch.abs(edge_s) + 1e-20).log()      # torch.Size([12096, 4])

        x_deq = x_deq.view(batch_size, -1)          #torch.Size([32, 342])       (batch, N * 9)
        adj_deq = adj_deq.view(batch_size, -1)      #torch.Size([32, 1512])      (batch, (repeat_num-N) * 4)


        x_log_jacob = x_log_jacob.view(batch_size, -1).sum(-1)          # (batch)
        adj_log_jacob = adj_log_jacob.view(batch_size, -1).sum(-1)      # (batch)

        return [x_deq, adj_deq], [x_log_jacob, adj_log_jacob], global_vector
    ...
    def _get_embs(self, x, adj):
        ...
        graph_node_emb_edge = graph_node_emb_edge.view(batch_size * (self.repeat_num - self.graph_size), -1)
