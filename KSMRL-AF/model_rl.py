...
class GraphFlowModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.num_flow_layer = 12
        self.nhid = 128
        self.nout = 128
        with open('./config/qed.json') as f:
            self.conf_rl = json.load(f)['model']['rl_conf_dict']
        self.deq_coeff = args.deq_coeff
    ...
    def reinforce_optim_one_mol(self, atom_list, temperature=0.75, max_size_rl=38):
        ...
        with torch.no_grad():
            ...
            for i in range(max_size_rl):
                ...
                if self.dp:
                    latent_node = self.flow_core_old.module.reverse(cur_node_features, cur_adj_features, latent_node,
                                                                    mode=0).view(-1)  # (9, )
                else:
                    latent_node = self.flow_core_old.reverse(cur_node_features, cur_adj_features, latent_node,
                                                             mode=0).view(-1)  # (9, )
                ...
                for j in range(edge_total):
                    ...
                    while not valid:
                        if len(
                            ...
                            if self.dp:
                                latent_edge = self.flow_core_old.module.reverse(cur_node_features, cur_adj_features,
                                                                                latent_edge,
                                                                                mode=1, edge_index=torch.Tensor(
                                        [[j + start, i]]).long().cuda()).view(-1)  # (4, )
                            else:
                                latent_edge = self.flow_core_old.reverse(cur_node_features, cur_adj_features,
                                                                         latent_edge, mode=1,
                                                                         edge_index=torch.Tensor(
                                                                             [[j + start, i]]).long().cuda()).view(
                                    -1)  # (4, )
    ...
    def reinforce_forward_optim(self, in_baseline=None, cur_iter=None):
        ...
        atom_list, temperature, batch_size, max_size_rl = self.conf_rl['atom_list'], self.conf_rl['temperature'], \
                                                          self.conf_rl['batch_size'], self.conf_rl['max_size_rl']
        ...
        max_action_size = batch_size * (int(max_size_rl + (self.edge_unroll - 1) * self.edge_unroll / 2 + (
            ...
            while total_node_step + total_edge_step < max_action_size and batch_length < batch_size:
                ...
                for i in range(max_size_rl):
                    ...
                    for j in range(edge_total):
                        ...
                        while not valid:
                            ...
                            traj_adj_inputs['edge_cnt'].append(
                                torch.full(size=(1,), fill_value=edge_idx, dtype=torch.long).long().cuda())
                ...
                if not final_valid:
                    ...
                    try:
                        ...
                        if self.conf_rl['reward_type'] == 'exp':
                            reward_property += (
                                        np.exp(score / self.conf_rl['exp_temperature']) - self.conf_rl['exp_bias'])
                ...
                if num_atoms > 1:
                    ...
                    traj_adj_inputs_rewards[traj_adj_inputs_rewards > 0] = \
                        reward_final_total * torch.pow(reward_decay, step_cnt - 1. - traj_adj_inputs_rewards[
                            traj_adj_inputs_rewards > 0])
        ...
        if in_baseline is not None:
            ...
            reward_baseline_per_step = reward_baseline_per_step * (1. - self.conf_rl['moving_coeff']) + in_baseline * \
                                       self.conf_rl['moving_coeff']
        ...
        node_inputs_node_features_cont += self.deq_coeff * torch.rand(node_inputs_node_features_cont.size(),
                                                                      device='cuda:%d' % (
                                                                          node_inputs_node_features_cont.get_device()))  # (total_size, 9)
        adj_inputs_edge_features_cont += self.deq_coeff * torch.rand(adj_inputs_edge_features_cont.size(),
                                                                     device='cuda:%d' % (
                                                                         adj_inputs_edge_features_cont.get_device()))  # (total_size, 4)
        ...
        if not self.conf_rl['split_batch']:
            ...
            z_node1, logdet_node1 = node_function(node_inputs_node_features[:mid_node],
                                                  node_inputs_adj_features[:mid_node],
                                                  node_inputs_node_features_cont[
                                                  :mid_node])  # (total_step, 9), (total_step, )
            z_node2, logdet_node2 = node_function(node_inputs_node_features[mid_node:],
                                                  node_inputs_adj_features[mid_node:],
                                                  node_inputs_node_features_cont[
                                                  mid_node:])  # (total_step, 9), (total_step, )
            ...
            z_edge1, logdet_edge1 = edge_function(adj_inputs_node_features[:mid_edge],
                                                  adj_inputs_adj_features[:mid_edge],
                                                  adj_inputs_edge_features_cont[:mid_edge],
                                                  adj_inputs_index[:mid_edge])  # (total_step, 4), (total_step, )
            z_edge2, logdet_edge2 = edge_function(adj_inputs_node_features[mid_edge:],
                                                  adj_inputs_adj_features[mid_edge:],
                                                  adj_inputs_edge_features_cont[mid_edge:], adj_inputs_index[
                                                                                            mid_edge:])  # (total_step, 4), (total_step, )
        ...
        with torch.no_grad():
            if not self.conf_rl['split_batch']:
                ...
                mid_node = node_inputs_node_features.size(0) // 2
                z_node_old1, logdet_node_old1 = node_function_old(node_inputs_node_features[:mid_node],
                                                                  node_inputs_adj_features[:mid_node],
                                                                  node_inputs_node_features_cont[
                                                                  :mid_node])  # (total_step, 9), (total_step, )
                z_node_old2, logdet_node_old2 = node_function_old(node_inputs_node_features[mid_node:],
                                                                  node_inputs_adj_features[mid_node:],
                                                                  node_inputs_node_features_cont[
                                                                  mid_node:])  # (total_step, 9), (total_step, )
                ...
                mid_edge = adj_inputs_node_features.size(0) // 2
                z_edge_old1, logdet_edge_old1 = edge_function_old(adj_inputs_node_features[:mid_edge],
                                                                  adj_inputs_adj_features[:mid_edge],
                                                                  adj_inputs_edge_features_cont[:mid_edge],
                                                                  adj_inputs_index[
                                                                  :mid_edge])  # (total_step, 4), (total_step, )
                z_edge_old2, logdet_edge_old2 = edge_function_old(adj_inputs_node_features[mid_edge:],
                                                                  adj_inputs_adj_features[mid_edge:],
                                                                  adj_inputs_edge_features_cont[mid_edge:],
                                                                  adj_inputs_index[
                                                                  mid_edge:])  # (total_step, 4), (total_step, )
        ...
        ll_node_old = -1 / 2 * (torch.log(2 * self.constant_pi) + self.prior_ln_var + torch.exp(-self.prior_ln_var) * (
                    z_node_old ** 2))
        ll_node_old = ll_node_old.sum(-1)  # (B)
        ll_edge_old = -1 / 2 * (torch.log(2 * self.constant_pi) + self.prior_ln_var + torch.exp(-self.prior_ln_var) * (
                    z_edge_old ** 2))
        ...
        return -((torch.min(surr1_node, surr2_node).sum() + torch.min(surr1_edge, surr2_edge).sum()) / (
